 ...
 *   slider <name> <min> <max> [<unit>] <"template"> [=default] [@weight]
 ...
 * clamped or ignored. `@weight` and `=default` may appear in either
 * order after the template.
 *
 * Windows are live-resizable: sliders, fields, toggles, buttons, and
 * choices all track the window's width as it's resized (their x/width
 * are recomputed from the same column-splitting logic used at load
 * time). Row heights and vertical layout stay fixed -- only horizontal
 * space is redistributed. A minimum window size is enforced so controls
 * can't be crushed by shrinking too far.
 *
