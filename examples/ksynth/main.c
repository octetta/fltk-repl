/* =========================================================================
 * krepl - K-Synth Array DSP REPL  v0.7
 *
 * fltk-repl front-end for the K-Synth engine with full panel DSL support.
 *
 * Features:
 *  - Up to 256 polyphonic voices (default 16)
 *  - Global master gain + tanh soft-limiter (prevents overdrive)
 *  - Pitch transposition (semitones + cents) and per-voice gain
 *  - 16 sample slots (assign, play, clear, list)
 *  - Panel DSL with @ksynth / @skred / @end embedded code blocks
 *  - Multi-line paste: splits on \n, evaluates each line independently
 *  - Audio output device enumeration and runtime selection (\D, \O)
 *  - Load audio files into slots (\L) or K variables (\F)
 *
 * Key REPL commands:
 *   \p [s] VAR [+/-semi[:cents]] [gain]   play variable
 *   \q                                    stop all voices
 *   \x                                    voice status
 *   \g [VAL]                              get/set master gain (0-4, default 0.5)
 *   \V N                                  set polyphony (1-256)
 *   \A N VAR [s]                          assign var to slot 1-16
 *   \P N [+/-semi[:cents]] [gain]         play slot N
 *   \b                                    list all slots
 *   \k N                                  clear slot N
 *   \D                                    list audio output devices
 *   \O N                                  open device N from \D list
 *   \L N FILE                             load audio file into slot N
 *   \F VAR FILE                           load audio file into variable
 *   \s [s] VAR                            save WAV
...
#define MAX_VOICES_CAP 256

/* Voice: phase = fractional frame index; rate = pitch multiplier. */
typedef struct {
    K      buffer;
    double phase;
    double rate;
...
typedef struct {
    float *buf;
    int    len;       /* total samples (frames*2 for stereo) */
    int    stereo;
    char   label[48];
} Slot;
...
static ma_context     g_ma_context; /* for device enumeration */
static int            g_ma_ctx_ok   = 0;
static int            g_show        = 0;
static int            g_audio_ok    = 0;
/* Master gain applied after mixing; default 0.5 gives 6 dB headroom. */
static volatile float g_master_gain = 0.5f;
...
static void audio_cb(ma_device *d, void *out, const void *in, ma_uint32 n) {
    ...
    /* Master gain + fast tanh soft-limiter: x*(27+x²)/(27+9x²) */
    float mg = g_master_gain;
    for (ma_uint32 j = 0; j < n * 2; j++) {
        float s = o[j] * mg;
        float s2 = s * s;
        o[j] = s * (27.0f + s2) / (27.0f + 9.0f * s2);
    }
}

/* ---- device management --------------------------------------------------- */

/* Reopen the audio device using device_id (NULL = default). */
static int open_audio_device(const ma_device_id *device_id) {
    if (g_audio_ok) {
    ...
    if (device_id) dcfg.playback.pDeviceID = device_id;
    if (ma_device_init(g_ma_ctx_ok ? &g_ma_context : NULL, &dcfg, &g_device) == MA_SUCCESS) {
...
static void voice_start(K buf, int stereo, double rate, float gain) {
    ...
    if (vslot < 0) {
        repl_printf(g_repl, "/ no free voice slots (polyphony=%d, \\V N to increase)\n", g_n_voices);
        k_free(g_ctx, buf); return;
    }
    ...
    if (semi == 0 && centi == 0)
        ...
        repl_printf(g_repl, "/ voice %d: %s  %+d semitones %+d cents  gain=%.2f  %.1fms\n",
            vslot, stereo ? "stereo" : "mono  ", semi, centi, gain,
            (double)g_voices[vslot].n_frames / 44100.0 * 1000.0);
}

/* ---- audio file loader (miniaudio decoder) -------------------------------- */
/*
 * Loads any file miniaudio can decode (WAV, MP3, FLAC, OGG, etc.) and
 * resamples to 44100 Hz f32.  Returns a malloc'd float* and sets *out_len
 * to total sample count and *out_stereo to 1 if 2-channel.
 * Caller owns the buffer.
 */
static float *load_audio_file(const char *path, int *out_len, int *out_stereo) {
    ma_decoder_config cfg = ma_decoder_config_init(ma_format_f32, 0, 44100);
    ma_decoder dec;
    if (ma_decoder_init_file(path, &cfg, &dec) != MA_SUCCESS) {
        repl_printf(g_repl, "/ cannot decode: %s\n", path);
        return NULL;
    }
    ma_uint64 total_frames;
    if (ma_decoder_get_length_in_pcm_frames(&dec, &total_frames) != MA_SUCCESS || total_frames == 0) {
        /* fallback: read into growing buffer */
        size_t cap = 44100 * 2 * sizeof(float);
        float *buf = (float *)malloc(cap);
        if (!buf) { ma_decoder_uninit(&dec); return NULL; }
        ma_uint64 total_read = 0;
        ma_uint32 chans = dec.outputChannels;
        while (1) {
            float tmp[4096 * 2];
            ma_uint64 read;
            ma_decoder_read_pcm_frames(&dec, tmp, 4096, &read);
            if (read == 0) break;
            size_t need = (total_read + read * chans) * sizeof(float);
            if (need > cap) {
                while (cap < need) cap *= 2;
                float *nb = (float *)realloc(buf, cap);
                if (!nb) { free(buf); ma_decoder_uninit(&dec); return NULL; }
                buf = nb;
            }
            memcpy(buf + total_read, tmp, read * chans * sizeof(float));
            total_read += read * chans;
        }
        *out_len    = (int)total_read;
        *out_stereo = (chans == 2) ? 1 : 0;
        ma_decoder_uninit(&dec);
        return buf;
    }
    ma_uint32 chans = dec.outputChannels;
    size_t nsamp = (size_t)total_frames * chans;
    float *buf = (float *)malloc(nsamp * sizeof(float));
    if (!buf) { ma_decoder_uninit(&dec); return NULL; }
    ma_uint64 read;
    ma_decoder_read_pcm_frames(&dec, buf, total_frames, &read);
    ma_decoder_uninit(&dec);
    *out_len    = (int)(read * chans);
    *out_stereo = (chans == 2) ? 1 : 0;
    return buf;
}
...
static void draw_line_grid(unsigned char *grid, int x1, int y1, int x2, int y2, int w, int h) {
    int dx=abs(x2-x1),sx=x1<x2?1:-1,dy=-abs(y2-y1),sy=y1<y2?1:-1,err=dx+dy,e2;
    while(1){
...
static void print_scope_to_repl(double *data, int len, int width, int height) {
    ...
    for(int i=1;i<len;i++){if(data[i]<min_y)min_y=data[i];if(data[i]>max_y)max_y=data[i];}
    ...
    char hdr[256];
    snprintf(hdr,sizeof(hdr),"\x1b[38;5;240m  MAX:%-10.4f DUR:%.3fms / %d samples\x1b[0m",
    ...
    for(int y=0;y<ch;y+=4){
        ...
        for(int x=0;x<cw;x+=2){
            unsigned int mask=0;
            for(int i=0;i<8;i++){int dy=y+dots[i][0],dx=x+dots[i][1];
                if(dy<ch&&dx<cw&&grid[dy*cw+dx])mask|=(unsigned)bits[i];}
            if(mask==0){strcat(row,has_zero?"\x1b[38;5;244m\xe2\x80\xa5\x1b[0m":" ");}
            else{unsigned int cp=0x2800u+mask;char cell[8];
...
static void show_var(char v_name) {
    ...
    for (int i = 0; i < lim; i++)
        repl_printf(g_repl, "%.4f%s", v->f[i], i == lim-1 ? "" : " ");
...
static void panel_error_cb(const char *msg, void *ud) {
    (void)ud; if (g_repl && msg) repl_printf(g_repl, "\x1b[31m/ panel: %s\x1b[0m\n", msg);
}
static void panel_list_cb(const char *name, const char *path, const char *params, int is_shown, void *ud) {
    (void)ud; (void)params;
...
static char *preprocess_panel(const char *path) {
    ...
    if (!fout) { fclose(fin); repl_println(g_repl, "/ cannot write temp panel"); return NULL; }
    char line[4096]; int in_block = 0; /* 1=ksynth, 2=skred */
    while (fgets(line, sizeof(line), fin)) {
        size_t len = strlen(line);
        char trimmed[4096]; memcpy(trimmed, line, len + 1);
        while (len > 0 && (trimmed[len-1] == '\n' || trimmed[len-1] == '\r')) trimmed[--len] = '\0';
        const char *t = trimmed; while (*t == ' ' || *t == '\t') t++;
        if (strncmp(t, "@ksynth", 7) == 0 && (t[7] == '\0' || isspace((unsigned char)t[7]))) {
            in_block = 1; repl_printf(g_repl, "/ panel @ksynth block\n"); continue;
        }
        if (strncmp(t, "@skred", 6) == 0 && (t[6] == '\0' || isspace((unsigned char)t[6]))) {
            in_block = 2; repl_printf(g_repl, "/ panel @skred block (skipped in krepl)\n"); continue;
        }
        if (strcmp(t, "@end") == 0) {
            if (in_block == 1) repl_printf(g_repl, "/ @ksynth block done\n");
            in_block = 0; continue;
        }
        if (in_block == 1) handle_line(trimmed);
        else if (in_block != 2) fputs(line, fout);
    }
...
static void krepl_panel_load(const char *pname, const char *ppath, const char *params) {
    ...
    unlink(tmp); free(tmp);
    if (pw) { panel_registry_show(pname); repl_printf(g_repl, "/ panel '%s' loaded from %s\n", pname, ppath); }
    else      repl_printf(g_repl, "/ panel load failed: %s\n", ppath);
}
...
static void handle_panel_cmd(const char *rest) {
    ...
    if (sscanf(rest, "%31s%n", sub, &consumed) < 1) {
        repl_println(g_repl, "/ usage: panel load|open|reload|show|hide|list|set|get|dump"); return;
    }
    ...
    if (strcmp(sub, "load") == 0) {
        ...
        if (sscanf(arg, "%63s %255s%n", pname, ppath, &nc) >= 2) {
            ...
            if (*extra) {
                ...
                while (*extra && left) {
                    if (*extra==' '){if(p>params){*p++=',';left--;}extra++;}else{*p++=*extra++;left--;}
                } *p = '\0';
            }
            krepl_panel_load(pname, ppath, params);
        } else repl_println(g_repl, "/ usage: panel load <name> <file.pnl> [key=val ...]");
        ...
        if (picked) { krepl_panel_load(pname, picked, NULL); repl_free_string(picked); }
    ...
    } else if (strcmp(sub, "show") == 0) { panel_registry_show(arg); repl_printf(g_repl,"/ panel '%s' shown\n",arg);
    } else if (strcmp(sub, "hide") == 0) { panel_registry_hide(arg); repl_printf(g_repl,"/ panel '%s' hidden\n",arg);
    } else if (strcmp(sub, "list") == 0) { repl_println(g_repl,"/ panels:"); panel_registry_list(panel_list_cb,NULL);
        ...
        if (sscanf(arg,"%63s %63s %127s %d",pname,ctrl,val,&fire)>=3) {
            ...
            else repl_printf(g_repl,"/ set failed: '%s.%s'\n",pname,ctrl);
        ...
        if (sscanf(arg,"%63s %63s",pname,ctrl)==2) {
            if (panel_registry_get_value(pname,ctrl,buf,sizeof(buf))==0)
                repl_printf(g_repl,"/ %s.%s = %s\n",pname,ctrl,buf);
            else repl_printf(g_repl,"/ get failed: '%s.%s'\n",pname,ctrl);
    ...
    } else repl_printf(g_repl,"/ unknown panel subcommand: %s\n",sub);
}

/* ---- device list callback ------------------------------------------------ */

/* We store device IDs so \O N can pick one. */
#define MAX_ENUM_DEVS 64
static ma_device_id g_dev_ids[MAX_ENUM_DEVS];
...
static void handle_backslash_cmd(const char *line) {
    ...
    if (cmd == '?') {
        repl_println(g_repl,
            "\\p [s] VAR [+/-semi[:cents]] [gain]   play variable\n"
            "\\q                                    stop all voices\n"
            "\\x                                    voice + device status\n"
            "\\g [VAL]                              get/set master gain (0.0-4.0)\n"
            "\\V N                                  set polyphony (1-256)\n"
            "\\e [FILE]                             open script editor (also: edit [file])\n"
            "\\A N VAR [s]                          assign var to slot 1-16\n"
            "\\P N [+/-semi[:cents]] [gain]         play slot N\n"
            "\\b                                    list all slots\n"
            "\\k N                                  clear slot N\n"
            ...
            "\\l [FILE]                             load .ks script\n"
            "\\w MS                                 wait milliseconds\n"
            "\\t                                    toggle scope-after-eval\n"
            "\\?                                    this help\n"
            "\npanel load|open|reload|show|hide|list|set|get|dump ...\n"
            "\n@ksynth / @skred / @end  (embedded panel init blocks)\n"
            "pitch: +7=fifth  -12=octave down  +3:50=3.5 semitones\n"
            "gain:  0.5=-6dB  2.0=+6dB  1.0=unity");

    } else if (cmd == 't') {
        g_show = !g_show;
        repl_printf(g_repl, "/ scope %s\n", g_show ? "on" : "off");

    } else if (cmd == 'g') {
        const char *arg = line + 2; while (*arg == ' ') arg++;
        if (*arg == '\0') {
            repl_printf(g_repl, "/ master gain: %.3f\n", (double)g_master_gain);
        } else {
        ...
        const char *arg = line + 2; while (*arg == ' ') arg++;
        int n = atoi(arg);
        if (n < 1) { repl_println(g_repl, "/ \\V: must be >= 1"); return; }
        if (n > MAX_VOICES_CAP) { n = MAX_VOICES_CAP; repl_printf(g_repl, "/ clamped to %d\n", n); }
        int old = g_n_voices;
        if (set_polyphony(n) == 0) repl_printf(g_repl, "/ polyphony: %d -> %d\n", old, g_n_voices);
        else repl_println(g_repl, "/ \\V: allocation failed");

    } else if (cmd == 'D') {
        /* \D — enumerate output devices */
        if (!g_ma_ctx_ok) { repl_println(g_repl, "/ audio context unavailable"); return; }
        ma_device_info *pinfo; ma_uint32 count;
        ma_device_info *pcinf; ma_uint32 ccount;
        if (ma_context_get_devices(&g_ma_context, &pinfo, &count, &pcinf, &ccount) != MA_SUCCESS) {
            repl_println(g_repl, "/ device enumeration failed"); return;
        }
        g_dev_count = (int)count < MAX_ENUM_DEVS ? (int)count : MAX_ENUM_DEVS;
        repl_printf(g_repl, "/ %d output device(s):\n", g_dev_count);
        for (int i = 0; i < g_dev_count; i++) {
            g_dev_ids[i] = pinfo[i].id;
            repl_printf(g_repl, "  [%d] %s%s\n", i, pinfo[i].name,
                        pinfo[i].isDefault ? "  \x1b[32m(default)\x1b[0m" : "");
        }
        repl_println(g_repl, "/ use \\O N to switch");

    } else if (cmd == 'O') {
        /* \O N — open device N */
        const char *arg = line + 2; while (*arg == ' ') arg++;
        int n = atoi(arg);
        if (n < 0 || n >= g_dev_count) {
            repl_printf(g_repl, "/ \\O: run \\D first (0..%d)\n", g_dev_count - 1); return;
        }
        /* stop all voices before switching */
        for (int i = 0; i < g_n_voices; i++) {
            g_voices[i].active = 0;
            if (g_voices[i].buffer) { k_free(g_ctx, g_voices[i].buffer); g_voices[i].buffer = NULL; }
        }
        if (open_audio_device(&g_dev_ids[n]) == 0)
            repl_printf(g_repl, "/ opened device %d\n", n);
        else
            repl_printf(g_repl, "/ failed to open device %d\n", n);

    } else if (cmd == 'L') {
        /* \L N FILE — load audio file into slot N */
        const char *arg = line + 2; while (*arg == ' ') arg++;
        int n = atoi(arg);
        if (n < 1 || n > NUM_SLOTS) { repl_printf(g_repl, "/ \\L: slot 1-%d\n", NUM_SLOTS); return; }
        while (*arg && !isspace((unsigned char)*arg)) arg++;
        while (*arg == ' ') arg++;
        if (!*arg) {
            /* file picker */
            char *picked = repl_open_file_dialog(g_repl, "Load Audio File",
                "Audio\t*.wav *.flac *.mp3 *.ogg\nAll Files\t*");
            if (!picked) return;
            int len, stereo;
            float *buf = load_audio_file(picked, &len, &stereo);
            if (buf) {
                Slot *sl = &g_slots[n-1]; free(sl->buf);
                sl->buf = buf; sl->len = len; sl->stereo = stereo;
                snprintf(sl->label, sizeof(sl->label), "%s",
                         strrchr(picked, '/') ? strrchr(picked, '/') + 1 : picked);
                repl_printf(g_repl, "/ slot %d <- %s [%d samples / %.1fms]%s\n",
                            n, sl->label, len, (double)len / (stereo ? 2.0 : 1.0) / 44100.0 * 1000.0,
                            stereo ? " stereo" : "");
            }
            repl_free_string(picked);
        } else {
            int len, stereo;
            float *buf = load_audio_file(arg, &len, &stereo);
            if (buf) {
                Slot *sl = &g_slots[n-1]; free(sl->buf);
                sl->buf = buf; sl->len = len; sl->stereo = stereo;
                const char *base = strrchr(arg, '/') ? strrchr(arg, '/') + 1 : arg;
                snprintf(sl->label, sizeof(sl->label), "%s", base);
                repl_printf(g_repl, "/ slot %d <- %s [%d samples / %.1fms]%s\n",
                            n, sl->label, len, (double)len / (stereo ? 2.0 : 1.0) / 44100.0 * 1000.0,
                            stereo ? " stereo" : "");
            }
        ...
        /* \F VAR FILE — load audio file into K variable */
        const char *arg = line + 2; while (*arg == ' ') arg++;
        char v_name = get_var_name(arg);
        if (!v_name) { repl_println(g_repl, "/ \\F: no variable"); return; }
        while (*arg && !isspace((unsigned char)*arg)) arg++;
        while (*arg == ' ') arg++;
        const char *filepath = arg;
        char *picked = NULL;
        if (!*filepath) {
            picked = repl_open_file_dialog(g_repl, "Load Audio File",
                "Audio\t*.wav *.flac *.mp3 *.ogg\nAll Files\t*");
            if (!picked) return;
            filepath = picked;
        }
        int len, stereo;
        float *buf = load_audio_file(filepath, &len, &stereo);
        if (buf) {
            /* allocate a K array and fill it */
            K kv = k_new_perm(g_ctx, len);
            if (kv) {
                for (int i = 0; i < len; i++) kv->f[i] = (double)buf[i];
                if (g_ctx->vars[v_name-'A']) k_free(g_ctx, g_ctx->vars[v_name-'A']);
                g_ctx->vars[v_name-'A'] = kv;
                repl_printf(g_repl, "/ %c <- %s [%d samples / %.1fms]%s\n",
                            v_name, filepath, len,
                            (double)len / (stereo ? 2.0 : 1.0) / 44100.0 * 1000.0,
                            stereo ? " stereo" : "");
            }
            free(buf);
        }
        if (picked) repl_free_string(picked);
        ...
        repl_printf(g_repl, "/ polyphony: %d  master gain: %.3f  audio: %s\n",
                    g_n_voices, (double)g_master_gain, g_audio_ok ? "ok" : "off");
        int any = 0;
        for (int i = 0; i < g_n_voices; i++) {
            if (!g_voices[i].active || !g_voices[i].buffer) continue;
            int nf = g_voices[i].n_frames;
            int pct = (int)(g_voices[i].phase / nf * 100.0);
            double sf = log2(g_voices[i].rate) * 12.0;
            int semi = (int)round(sf), centi = (int)round((sf - semi) * 100.0);
            repl_printf(g_repl, "  [%d] %s  %d/%d (%d%%)  %+d:%+d semi  gain %.2f\n",
                i, g_voices[i].stereo ? "stereo" : "mono  ",
                (int)g_voices[i].phase, nf, pct, semi, centi, g_voices[i].gain);
            any = 1;
        }
        if (!any) repl_println(g_repl, "  (no active voices)");
        ...
        while (*arg == ' ') arg++;
        int stereo = 0;
        if (*arg == 's') { stereo = 1; arg++; while (*arg == ' ') arg++; }
        char v_name = get_var_name(arg);
        ...
        for (int i = 0; i < NUM_SLOTS; i++) {
            Slot *sl = &g_slots[i];
            if (sl->buf)
                repl_printf(g_repl, "  \x1b[36m[%2d]\x1b[0m %-12s %6d samples / %7.1fms%s\n",
                    i+1, sl->label, sl->len,
                    (double)sl->len / (sl->stereo ? 2.0 : 1.0) / 44100.0 * 1000.0,
                    sl->stereo ? " stereo" : "");
            else
        ...
        int col = 0;
        for (int i = 0; i < v->n; i++) {
            fprintf(f, "%g, ", v->f[i]); if (++col >= 64) { fprintf(f, "\n"); col = 0; }
        }
        if (col) fprintf(f, "\n"); fprintf(f, "};\n"); fclose(f);
        ...
        } else { char v_name = get_var_name(line + 2); if (v_name) show_var(v_name); }
        ...
        char wt[32]; snprintf(wt, sizeof(wt), "waveform: %c", v_name);
        bitmap_win_t *bw = bitmap_win_get(wt);
        bitmap_win_set_waveform_ex(bw, buf, v->n, 1, 0, 800, 300, wt, -1, -1, 44100.0f);
        bitmap_win_show(bw); free(buf);
        repl_printf(g_repl, "/ waveform: %c [%d samples]\n", v_name, v->n);
        ...
        if (!v) { repl_printf(g_repl, "/ %c is empty\n", v_name); return; }
        float *buf = k_to_float(v); if (!buf) return;
        char zt[32]; snprintf(zt, sizeof(zt), "spectrogram: %c", v_name);
        bitmap_win_t *bw = bitmap_win_get(zt);
        bitmap_win_set_spectrogram_labeled_ex(bw, buf, v->n, 1, 0, 800, 300, zt, 44100.0f);
        bitmap_win_show(bw); free(buf);
        repl_printf(g_repl, "/ spectrogram: %c [%d samples]\n", v_name, v->n);

    } else if (cmd == 'e') {
        /* \e [FILE]  — open the built-in text editor (same as 'edit' command) */
        const char *arg = line + 2; while (*arg == ' ') arg++;
        editor_win *ew = (*arg)
            ? repl_editor_open_file(g_repl, arg)
            : repl_editor_open(g_repl);
        (void)ew; /* window manages its own lifetime */
...
static void line_fallback(const char *line, void *ud) {
    (void)ud;
    if (!line || !*line) return;
    /* Multi-line paste: split on embedded newlines, evaluate each line
     * independently — as if typed one at a time. */
    if (strchr(line, '\n')) {
        char *tmp = strdup(line);
        if (!tmp) { handle_line(line); return; }
        char *p = tmp;
        while (*p) {
            char *nl = strchr(p, '\n');
            if (nl) *nl = '\0';
...
int main(int argc, char **argv) {
    ...
    /* initialise a ma_context for device enumeration (survives device switches) */
    if (ma_context_init(NULL, 0, NULL, &g_ma_context) == MA_SUCCESS)
        g_ma_ctx_ok = 1;

    if (open_audio_device(NULL) != 0)
        fprintf(stderr, "krepl: warning: audio device unavailable\n");
    ...
    repl_register_default_commands(g_repl);  /* gives 'edit [file]', 'help', etc. */
    panel_set_command_handler(panel_to_krepl, NULL);
    panel_set_error_handler(panel_error_cb, NULL);

    repl_println(g_repl, "\x1b[1;36mkrepl 0.7  K-Synth DSP REPL\x1b[0m");
    repl_printf(g_repl,  "/ audio: %s  voices: %d  master gain: %.2f\n",
                g_audio_ok ? "ok (44100Hz stereo)" : "unavailable",
                g_n_voices, (double)g_master_gain);
    repl_println(g_repl, "/ \\? help  \\e to open editor  \\D list devices  \\O N switch  \\L N file  \\F V file");
    ...
    if (g_ma_ctx_ok) ma_context_uninit(&g_ma_context);
    free(g_voices);
